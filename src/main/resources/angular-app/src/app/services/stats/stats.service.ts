...
export class StatsService {
  ...
  findcharacterPvpStats(nickname: string, characterId: string) :Observable<RootObject>{
    return this.httpClient.get<RootObject>('findplayer/' + nickname + '/pvpstats/' + characterId);
  }

  findcharacterPveStats(nickname: string, characterId: string) {
    return this.httpClient.get<RootObject>('findplayer/' + nickname + '/pvestats/' + characterId);
  }
}
